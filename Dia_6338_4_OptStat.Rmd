...
1. méthode de "direction set"
    ...
    a. direction opposée au gradient
    a. direction Newton
    a. direction BFGS

### Mise en oeuvre en R et résultats

## Méthode de dichotomie (bracketing)
...
\]
- Le maximum de $g$ résoud l'équation $g'(x) = f'(x^k) + f''(x^k)(x-x^k) = 0$.
- La solution $x^{k+1} - x^k = -f'(x^k)/f''(x^k)$.
- Notez que
\[
  -\frac{f'(x)}{f''(x)} = \frac{1-e^x}{e^x}.
\]
- Pour $x \ll 0$, $x^{k+1} - x^k \approx e^{-x} \gg |x|$.
- Pour $x \gg 0$, $x^{k+1} - x^k \approx -1$.
...
- Attention : en plusieurs dimensions $H_k (x^k - x^{k-1}) = \nabla f(x^k) - \nabla f(x^{k-1})$ donne $n$ équations, pas assez pour déterminer $H_k$.
...
- Notez que $\log(Y^\alpha) = \alpha y$ alors $Y^\alpha = e^{\alpha y}$.
...
- On fournit seulement une fonction pour évaluer la fonction (pas de gradient, pas de matrice hessienne).
...
Si $\|\nabla f(x^k)\| < \delta (1+f(x^k))$, réclamer le succès.
...
1. Coûts d'évaluation de $f$, $\nabla f$, $H(x^k)$.
1. Coût d'évaluation de $\nabla f$ relatif aux coûts d'évaluation de ces éléments :
    a. un cas extrème : $f(x) = f_1(x_1) + \ldots + f_n(x_n)$.
    a. un cas avec "rendements à l'échelle" $f(x) = g(h(x))$, avec $g$ et $h$ scalaires : $g'(h(x))$ est un factor commun du gradient.
1. Variations de $H(x)$ et la courbature de la fonction.
1. Régions de non-convexité.
1. Alignement des valeurs propres de la matrice hessienne et les axes de coordonnées.
1. Besoin de calculer la matrice hessienne de toute façon?

## Illustration des directions
...
- Les méthodes quasi-Newton utilisent une matrice $H_k$ au lieu de la matrice hessienne de la méthode Newton.
- Deux conditions convenables :
...
La mise à jour prend $O(n^2)$ opérations :
