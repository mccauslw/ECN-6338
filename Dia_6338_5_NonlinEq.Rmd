...
\]
où $f \colon \mathbb{R}^n \to \mathbb{R}$ et $f \in C^2$, est aussi la solution du système
\[
...
- La matrice jabobienne de $\frac{\partial f(x)}{\partial x^\top}$ est la matrice hessienne symmétrique de $f$.
...
- On évalue seuelement la fonction et non les dérivées.
...
\[
  ...
  y_k = f(x^{k+1}) - f(x^k).
\]
- Notez que $A_{k+1} s^k = A_k s^k + (y_k - A_k s^k) = y_k$, une condition de chorde.
