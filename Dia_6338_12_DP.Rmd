...
- Notes :
    ...
    - On peut inclure l'amortissement du capital dans $F$ : $F(k) = f(k) - \delta k$, où $f(k)$ est la fonction de production brute.
...
- Une fonction de politique est maintenant une fonction $U(x)$ qui vérifie
\[
...
\]
- Un sous-produit est une suite de fonctions de politique $C^l$ correspondantes:
\[
...
où $C(\cdot)$ est la fonction de commande optimale.
...
- La commande $C(k) = f(k)$ donne $k^+ = k$
- Elle est faisable mais pas optimale pour $k \neq k^{ss} = 1$.
- Autour de $k=k^{ss}$, $C(k)$ devrait être près de la commande optimale.
- La fonction de valeur (non-optimale) $V^c(k)$ associée à la commande $C(k) = f(k)$ (non-optimale) vérifie
\[
...
## La fonction de politique après deux itération de $V_c(k)$
...
## Code, itération de la fonction de politique

```{r PF_iteration}
# Iteration de la fonction de politique
...
## L'iteration de la fonction de politique
...
abline(h = uc + beta*vd$d, col='red') # Attn : h pour horizontal!
```
...
\[
  h(w) = \max(v(w), u(c) + \beta d).
\]
- Un agent qui a optimisé dans le passé ne devrait jamais être dans l'état $(1,w)$, si $w < \bar{w}$.
