...
Gibbs.sim <- function(M) {
  ...
  nu.bar.bar <- nu.bar + n; y.bar = mean(y)
  for (m in seq(2,M)) {
    s2.bar.bar <- s2.bar + sum((y-mu[m-1])^2)
    h[m] <- rchisq(1, nu.bar.bar) / s2.bar.bar

    omega.bar.bar <- omega.bar + h[m]*n
    mu.bar.bar <- (omega.bar*mu.bar+h[m]*n*y.bar)/
                  omega.bar.bar
    mu[m] <- rnorm(1, mu.bar.bar,
		               1/sqrt(omega.bar.bar))
  }
  list(mu=mu, h=h)
}
```
...
nu.bar=4; s2.bar=0.01; mu.bar=10; omega.bar=0.01
