...
- La loi cible est l'unique loi invariante de la transition de Markov suivante, de $\theta^{(m)}$ à $\theta^{(m+1)}$, où $\Sigma$ est n'importe quelle matrice définie positive :
...
- Supposez qu'il est difficile de simuler $\theta|y$.
...
Les deux premières femmes ci-bas sont dans la population active; les deux dernières, non.
