...
\]
    ...
    a. Démontrez que la densité $g(x)$ est non-négative et que $\int_0^\infty g(x)\, dx = 1$ pour tous $\pi \in (0,1)$. Donnez les chiffres pour $\alpha = 0.01, 0.1, 0.5, 0.9$. La fonction R \texttt{gamma} calcule la fonction $\Gamma(x)$.
    a. Trouvez
    \[
      M_1 \equiv \inf_{x \in [0,1]} \frac{f(x)}{g(x)}, \quad M_2 \equiv \inf_{x \in (1,\infty)} \frac{f(x)}{g(x)}.
    \]
    a. Trouvez $\pi \in (0,1)$ qui minimise $M = \min(M_1, M_2)$.
