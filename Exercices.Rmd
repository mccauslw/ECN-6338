...
Utilisez $M=10000$ tirages, $\bar{\beta} = 0$, $\bar{H} = 0.01I$, $\bar{s}^2 = 0.01$, $\bar{\nu} = 2$.
...
1. $k$ ans aprés la plantation d'un arbre, la quantité de bois vendable est
\[
  g(k) = e^{-\lambda} \sum_{j=0}^k \frac{\lambda^j}{j!}, \quad \lambda = 30.
\]
On peut utiliser la fonction R \texttt{ppois(k, lambda)} pour évaluer $g(k)$, mais notez qu'il n'y a pas d'interprétation stochastique; la forme sigmoïdique de la fonction de répartition d'une variable aléatoire poissonienne est qualitivement réaliste pour le croissance d'une arbre.
Un propriétaire possède une ferme avec une capacité d'un arbre.
À chaque période $t$, il peut couper l'arbre et vendre le bois à un prix réel de 1, ou laisser continuer à pousser l'arbre. Le taux d'intérêt réel est de $R = 5\%$.
Le propriétaire maximise la valeur actuelle de la récolte du bois.
Il commence avec un arbre d'âge $k_0 = 20$.
    a. Écrivez le problème dynamique du propriétaire dans la forme exposée dans la diapo "La version à horizon fini".
    a. Avec itération par fonction de valeur, trouver la fonction de valeur et la fonction de politique, deux fonctions sur $K \equiv \{0,1,\ldots,100\}$.
