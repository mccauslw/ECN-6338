...
1. Méthodes de type Dekker-Brent (sans dérivées)
1. Newton (dérivées)
1. Interpolation linéaire (approximation des dérivées)
...
1. Autres conditions de premier ordre pour l'optimisation
...
\[
  f(x) = 0,
\]
où $f \colon \mathbb{R} \to \mathbb{R}$. En d'autres termes, trouver une racine de $f(x)$.
...
Chaque firme choisit sa quantité en fonction de celle de l'autre.
L'équilibre (à la Cournot) du jeu du duopole est la paire
\[
...
1. Si $f(m)$ a le même signe que $f(a_k)$,
\[
...
- Considérez le jeu zéro-somme entre
    ...
    contenant une racine, et veut minimiser le nombre d'itérations ;
...
1. Calculer une proposition de repli $m = \frac{1}{2}(a_k + b_k)$.
...
\]
- Résoudre $f^i(x_1, x_2) = 0$ pour $x_i$, $i=1,2$, donne une mise à jour possible
de Seidel :
\[
