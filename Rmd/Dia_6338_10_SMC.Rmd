...
\[
  r \equiv \frac{n}{d} \equiv \frac{\sum_{m=1}^M w^{(m)} g(x^{(m)})}{\sum_{m=1}^M w^{(m)}} \overset{p}{\to} E_f[g(x)].
\]
...
- Dans un sens, l'échantillon pondéré $(w^{(m)}, x^{(m)})$ *représente* la loi cible : il y a une loi de grands nombres et un théorème central limite.
...
* Il faut simuler la loi avec densité $f^{(0)}(\theta)$.
    - Normalement, cette simulation est facile.
...
\]
1. Problème de filtrage, où $x_t$ est une suite d'états latents
\[
