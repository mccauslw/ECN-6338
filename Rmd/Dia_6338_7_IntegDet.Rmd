...
- Formules (ou règles) de Gauss pour l'intégration univariée
    - Règle de Gauss-Legendre, application aux enchères
    - Règle de Gauss-Laguerre, application à l'utilité actualisée
    - Règle de Gauss-Hermite, application aux espérances gaussiennes
...
```{r FenchereGraph}
plot(x, fv(x), type='l')    # densité en noir
lines(x, Fv(x), col='blue') # fct de répartition en bleu
```
...
## La version composite de la règle du point médian
...
\]
- Décomposer l'intégrale par intervalle et utiliser la règle du point médian, intervalle par intervalle, donne, pour un $\xi \in [a,b]$,
\[
...
- Pour la version composite,
    - tous les points d'évaluation, sauf les extrémités, servent à deux intervalles.
    - décomposer l'intervalle $[a,b]$ en $n$ sous-intervalles de longueur $h = \tfrac{b-a}{n}$.
...
- La règle de Simpson simple donne l'intégrale exacte d'une approximation quadratique
- La version composite donne, avec les mêmes $n+1$ évaluations que la version composite de la règle de trapèze avec $n$ intervalles de longueur $h$.
- La règle comparable d Simpson utilise $n/2$ intervalles de longueur $2h$ et $n$ doit être pair) et donne :
\[
  \begin{aligned}
...
| `I_me` | scalaire | l'intégrale cumulé $\int_{v_{\min}}^v F(x)^{n-1}\,dx$ au point actuel de la grille `v_me` |
| `b_me` | vecteur | valeurs de $b(v)$ sur le grille `v_me` |
| `I_tr`, \&c |  | Comme `I_me`, méthode de trapéze |
| `I_S`, \&c |  | Comme `I_S`, méthode de Simpson |
...
- Comme les règles de Newton-Cotes, les règles gaussiennes donnent des approximations
de la forme
\[
  \int_a^b w(x) f(x)\, dx \approx \sum_{i=1}^N \omega_i f(x_i),
\]
pour une collection de paires $(\omega_i,x_i)$.
- Résultat théorique  (Judd, théorème 7.2.1) : avec $N$ poids et $N$ noeuds les règles standards donnent la valeur exacte de l'intégrale pour les polynômes d'ordre
...
\]
une intégrale de la forme $\int_0^\infty e^{-x} g(x)\, dx$ (c.-à-d. $(a,b) = (0,\infty)$ et $w(x) = e^{-x}$), et donc disposé à approximation avec la règle Gauss-Laguerre.
...
## Valeurs de l'intégrale pour $N=3, 5, 10$
...
\]
une intégrale de la forme $\int_{-\infty}^\infty e^{-x^2} g(x)\, dx$ (c.-à-d. $(a,b) = (-\infty,\infty)$ et $w(x) = e^{-x^2}$), et donc disposé à approximation avec la règle Gauss-Hermite.
...
- Les règles de produit sont des solutions de force brute qui fonctionnent lorsque
...
- Le théorème de Mysovskikh garantit l'intégration exacte des polynôme d'ordre *total* $m$ avec un nombre comparable de points, mais ce n'est pas constructif.
