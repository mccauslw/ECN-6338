...
- Objectif : choisir $(u_1,\ldots,u_T)$ pour maximiser
\[
...
- Le modèle :
\[
  \max_{c_t} \sum_{t=0}^\infty \beta^t u(c_t) \quad \text{s.c.} \quad 0 \leq c_t \leq F(k_t), \; k_{t+1} = F(k_t) - c_t,
  \; k_0 \; \text{donné},
\]
...
- Problème d'inventaire agricole (Judd, page 428)
    - la décision est la quantité d'un intrant de production agricole et la quantité du produit à consommmer
    - l'état est l'inventaire agricole
    - l'état prochain est l'état actuel plus la production moins la consommation
...
La fonction valeur est comme une fonction d'utilité indirecte :
...
La fonction valeur pour le problème à horizon fini est
\[
...
- Une relation entre fonctions valeur de différentes périodes :
\[
  V(x,t) = \sup_{u \in D(x,t)} \pi(x,u,t) + V(F(x, u, t), t+1).
\]
- Si $u \in D(x,t)$ atteint le sup, la fonction politique est
\[
  U(x,t) = \arg\max_{u \in D(x,t)} \pi(x,u,t) + V(F(x, u, t), t+1).
\]
- $U(x,t)$, $V(x,t)$ sont des fonctions seulement de $x$ et $t$.
- La valeur de $x$ résume toute la mémoire du passé.

## Trouver la fonction valeur $V(x, 1)$
...
\[
  V(x,t) = \sup_{u \in D(x,t)} \pi(x,u,t) + V(F(x, u, t), t+1).
\]
- Attention : l'opérateur prend une fonction de $x$, $V(x,t+1)$, et donne une fonction de $x$, $V(x,t)$.
La valeur de $t$ est fixe et les fonctions $\pi(\cdot)$ et $F(\cdot)$ sont primitives.

## Le problème déterministe à horizon infinie

- La fonction valeur est maintenant définie de façon récursive :
\[
  V(x) = \sup_{u \in D(x)} \pi(x,u) + \beta V(F(x, u)),
\]
ou $V = TV$; $V$ est une fonction de $x \in X$, $T$ est un opérateur.
- L'équation est une équation *fonctionnelle*, sa solution est $V$, la *fonction* valeur.
- Une fonction politique est maintenant une fonction $U(x)$ qui vérifie
\[
  U(x) \in \arg\max_{u \in D(x)} \pi(x,u) + \beta V(F(x, u)).
\]

## Le théorème des applications contractantes et l'itération de la fonction valeur

- Le théorème des applications contractantes (contraction mapping theorem) :
si $0 < \beta < 1$, $\pi(x,u)$ est borné et $X$ est compacte, alors
    - $T$ est monotone ($V_1 \geq V_2 \Rightarrow TV_1 \geq TV_2$)
    - $T$ est une contraction de module $\beta$
    ($\|TV_1 - TV_2\| \leq \beta \|V_1 - V_2\|$)
    - $V = TV$ a une solution ($T$ a un point fixe) unique.
- L'itération de la fonction valeur donne une suite de fonctions valeur $V^l$ qui converge vers $V$ : $V^{l+1} = TV^l$, ou
\[
  V^{l+1}(x) = \sup_{u \in D(x)} \pi(x,u) + \beta V^l(F(x, u)).
\]
- Un sous-produit est une suite de fonctions politiques $U^l$ correspondantes:
\[
...
- La fonction valeur est un point fixe de l'opérateur $T$, où
\[
...
\[
  V(k) = \max_{k^+ \in K,\,k^+ \leq F(k)} u(F(k) - k^+) + \beta V(k^+).
\]
- Elle est un système d'équations non-linéaire ; la solution est un vecteur $(V(k^m),\ldots,V(k^M))$ qui vérifie les $|K|$ équations.
...
où $U(\cdot)$ est la fonction politique optimale.
- $\beta$ dans la fonction de production est étrange, le coefficient $(1-\beta)/(\alpha\beta)$ fait en sorte que $k^{ss} = 1$.

## Choisir une fonction valeur initiale

- Distinguez $U_c(k)$, la fonction politique qui donne la consommation actuelle, et $U_k(k)$, celle qui donne le capital prochain.
- Notez que $U_c(k) = f(k)$ donne $k^+ = k$ ou $U_k(k) = k$.
- Cette stratégie est faisable mais généralement sous-optimale lorsque $k \neq k^{ss} = 1$.
- Autour de $k=k^{ss}$, $U_c(k)$ et $U_k(k)$ devraient être près des fonctions politique optimales.
- La fonction valeur (non-optimale) $V^c(k)$ associée à $U_c(k) = f(k)$ (non-optimale) vérifie
\[
  V^c(k) = u(f(k)) + \beta V^c(k), \; \text{ou} \; V^c(k) = u(f(k))/(1-\beta).
\]
- Notez que $V^c(k) \leq V(k)$, où $V(k)$ est la fonction valeur associée à la fonction politique optimale.
- Considérez aussi $V_z(k) = 0$ comme fonction valeur initiale.
- Dans le code suivant, $U$ signifie $U_k$, le choix de capital comme fonction de capital.
...
# Fonction valeur initiale, épargne zéro
V_c <- u(f(k))/(1-beta)

# Fonction valeur initiale, zéro
V_z <- rep(0, N)
```

## Code, itération de la fonction valeur 

```{r VF_iteration, message=FALSE, warning=FALSE}
# Iteration de la fonction valeur
VU_suiv <- function(V) {
  V_suiv <- rep(0, N);
...
## La fonction valeur $TV_c(k)$ (deux itérations)
...
## La fonction politique après deux itération de $V_c(k)$
...
## Les fonctions valeur $T^{i}V_z(k)$, $i=1,2,3$
...
1. Itération de la fonction valeur : ce qu'on a déjà vu
1. Itération de la fonction politique : la matière prochaine

Pour faire l'itération de la fonction politique, il faut pouvoir calculer une approximation de la valeur de cette fonction politique comme fonction de l'état : pour une fonction politique $U \colon K \to \mathbb{R}$ donnée, il faut approximer sa valeur $V^U \colon K \to \mathbb{R}$.

## Approximation de $V^U$ pour $U_k$ donnée

- Pour une fonction politique $k^+ = U_k(k)$, pas forcément optimale,
on peut calculer, pour chaque $k_0 \in K$, la suite d'état
\[
  k_0, k_1 \equiv U_k(k_0), k_2 \equiv U_k^{(2)}(k_0), k_3 \equiv U_k^{(3)}(k_0), \ldots,
\]
où $U_k^{(0)}(k_0) = k_0$, $U_k^{(i+1)}(k_0) = U_k(U_k^{(i)}(k_0))$.
- On peut approximer la fonction valeur correspondante, le vecteur $V^U(k_0)$, $k_0 \in K$ par $\hat{V}^U(k_0)$ défini comme
\[
  \sum_{t=0}^T \beta^t u(k_t + f(k_t) - k_{t+1}) + \beta^{T+1}\tilde{V}(k_{(T+1)}),
\]
où $\tilde{V}$ est une approximation de la fonction valeur optimale.
...
VdeU <- function(VU, T) {
  # VU$V est W0
  # VU$U est la fonction politique qui donne k+
...
## Itération de la fonction politique

Itérez les étapes suivantes jusqu'à ce que $\|V^{l+1} - V^l\| < \epsilon$ :

1. Utilisez $V^l$ pour calculer $U^{l+1}$, la fonction politique obtenue comme sous-produit de l'itération de la fonction valeur.
1. Utilisez $U^{l+1}$ et $V^l$ (pour la valeur résiduelle à $t=T+1$) pour calculer $V^{l+1}$ comme
\[
  V^{l+1} \equiv \hat{V}^{U^{l+1}}
\]

## Iteration de la fonction politique (graphique)
...
- Notez la séparabilité additive de l'objectif :
...
- L'action $u_t$ est d'accepter ($u_t = 1$) l'offre $w_t$ et gagner $w_t$ chaque période à tout jamais ou de rejeter ($u_t = 0$) l'offre, gagner $c$ à $t$ et attendre une autre offre.
- L'agent maximise 
\[
...
## L'itération de la fonction valeur

- Dans un premier temps, on ne profite pas de la structure du problème.
- On commence par la fonction valeur qui correspond à la fonction politique $\sigma_0(w) = 1$, selon laquelle on accepte chaque offre :
\[
...
## L'iteration de la fonction valeur (graphique)
...
## Graphique de la fonction valeur $v(w)$ et de $u(c) + \beta d$
...
- $u(c) + \beta d$, la valeur de réservation, est en rouge.
