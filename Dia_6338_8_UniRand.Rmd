...
- Le modèle implique (pourvu que $E[f(X)]$ et $\mathrm{Var}[f(X)]$ existent)
\[
...
- Souvent, il y a un état $x_i$ qui suit une règle déterministe $x_i = f(x_{i-1},x_{i-2},\ldots)$ et une suite de nombres $y_i$ (fournit) qui suit une régle déterministe $y_i = g(x_i)$, où $g$ est une fonction à sens unique qui rend infaisable le calcul de l'état à partir des observations des $y_i$.
...
- Première idée alternative : choisir une loi exponentielle avec déplacement par $c$ :
\[
