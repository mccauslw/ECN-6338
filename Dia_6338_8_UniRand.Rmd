...
- Une suite *quasi-aléatoire* est justifiée par ses propriétés en théorie des nombres.
- Une suite *pseudo-aléatoire* est justifiée par la théorie de probabilité et la démonstration que elle se comporte comme une suite de variables aléatoires uniformes et indépéndentes.
- Les deux types de suite sont déterministes.
...
\]
- Cas univarié : $\{x_i\}_{i=1}^\infty \subseteq [a,b]$ est equidistribuée si pour chaque fonction $f \colon \mathbb{R} \to \mathbb{R}$, intégrable dans le sens de Riemann,
\[
  \lim_{n\to \infty} (b-a) \frac{1}{n} \sum_{i=1}^n f(x_i) = \int_a^b f(x)\, dx.
\]
- En pratique, on cherche un taux de convergence rapid.
...
## Suites Halton pour les trois premier nombres premiers
...
| 4   | $\tfrac{1}{8}$  | $0.001_2$  | $\tfrac{4}{9}$  | $0.11_3$  | $0.8$  | $0.5_5$  \vspace{0.15cm} |
...
- On utilise un vecteur de suites comme la suite $\{x_i\}_{i=1}^\infty \subset D$.
- Chaque suite univariée doit être équidistribuée sur $[0,1]$.
- Mais cela ne suffit pas pour construire une suite multivariée équidistribuée.
- La suite de [Halton](https://en.wikipedia.org/wiki/Halton_sequence) est populaire et utilise les $d$ premiers nombres premiers pour la construction.
- Suite de Weyl (\texttt{torus}, dans le paquet R \texttt{quasiRNG}) :
\[
  \{(i\sqrt{p_1} - \lfloor i\sqrt{p_1} \rfloor, i\sqrt{p_2} - \lfloor i\sqrt{p_2} \rfloor,
  \ldots, i\sqrt{p_d} - \lfloor i\sqrt{p_d} \rfloor)\},
\]
...
## D'autres implications de l'hypothèse $U_i \sim \mathrm{iid}\,U([0,1])$

- Admettons qu'on peut prouver que si $U_i \sim \mathrm{iid}\,U([0,1])$, la suite construite $\{X_i\}_{i=1}^\infty$ est iid avec densité cible $q(x)$.

- Une autre implication de l'hypothèse est que pour chaque $f\colon \mathbb{R}^d \to \mathbb{R}$ telle que $E[f(X)]$ et $\mathrm{Var}[f(X)]$ existent,
\[
