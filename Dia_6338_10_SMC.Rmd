...
- Le problème : intégrer des fonctions $g(x)$ par rapport à la densité $f(x)$,
lorsqu'on peut seulement évaluer un noyau $h(x) = f(x)/c$, où $c$ est une constante
inconnue.

- Une densité alternative (ou biaisée) est une $p(x)$ telle que
    - $p(x) > 0 \Leftrightarrow h(x) > 0$,
    - $\sup_x h(x)/p(x) < \infty$.
...
## Échantillonage préférentiel, cont.
...
- Et alors (par le théorème Mann-Wald)
\[
  \frac{\sum_{m=1}^M w^{(m)} g(x^{(m)})}{\sum_{m=1}^M w^{(m)}} \overset{p}{\to} E_f[g(x)].
\]

## Autres notes sur l'échantillonage préférentiel

- Les $w^{(m)}$ sont pareilles pour chaque choix de $g(\cdot)$.
...
- On peut faire les simulations de $p(x)$ par
    ...
    - Méthodes MCMC
- L'interprétation de l'intégration par l'échantillonage préférentiel comme un changement de variables est très intéressante.

## L'erreur numérique de l'échantillonage préférentiel
...
- Solution : échantillonnez $\theta_n^*$, $n=1,\ldots,N$, avec remise, de l'échantillon $\theta_n$, $n=1,\ldots,N$, avec les poids de probabilité $w_n$.
...
- Attention : le rééchantillonage ajoute du bruit. On le fait seulement si on a besoin d'un échantillon non-pondéré.
...
$k^{(0)}(\theta) = f^{(0)}$ est normalisée.

* Il est facile de simuler de la loi avec densité $f(\theta)$.

* Le dernier noyau de densité $k^{(L)}(\theta)$ est de la loi cible.

* Les loi intermédiaires peut être ou ne pas être de l'intérêt, mais
...
Les lois intermédiare peuvent être de l'intérêt dans les cas 2 et 4.
...
### Itération à faire pour $l=1,\ldots,L$

1. Phase C (Correction, avec échantillonage préférentiel)
    - L'intrant est un échantillon $(N^{-1}, \theta_n^{(l-1)})$ représente $f^{(l-1)}(\theta)$.
    - On calcule $w_n^{(l)} = k^{(l)}(\theta_n^{(l-1)})/k^{(l-1)}(\theta_n)$.
    - Le résultat est un échantilon $(w_n^{(l)}, \theta_n^{(l-1)})$ qui représente $k^{(l)}(\theta)$.

1. Phase S (Sélection, avec rééchantillonage)
    - On rééchantillone l'échantillon pondéré $(w_n^{(l)}, \theta_n^{(l-1)})$, qui représente $k^{(l)}(\theta)$.
...
1. À la phase $M$ de l'itération $l$, comment décider le nombre d'itérations $K_l$ dans la phase M et quelle transitions markoviennes à utiliser?
...
- Executer deux versions de l'algorithme, en ordre :
    ...
    1. Une version non-adaptive où on utilise toutes les décisions utilisées dans la version adaptative.
...
- Souvent, le parallélisme n'est pas seulement
    - "embarassant", dans le sens du manque de communication entre particules,
    - mais aussi SIMD, dans le sens qu'on fait les mêmes opérations (mais avec l'information locale) pour chaque particule.
