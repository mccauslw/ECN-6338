...
- Il faut transformer les fonctions $\varphi \colon [0,1] \to \mathbb{R}$ pour
avoir les fonctions $\varphi \colon [x_i, x_{i+1}] \to \mathbb{R}$.
...
- Supposons que les colonnes sont orthogonales (un cas très spécial, inhabituel)
- Alors,
\[
  X^\top X =
  \begin{bmatrix}
...
\]
- Notez les produits intérieurs.

## Familles de polynômes orthogonaux : produits intérieurs

- Il y a plusieurs produits intérieurs dans l'espace des fonctions,
y compris
\[
...
- Pour quelles valeurs de $\alpha$, $\beta$ est-ce que la fonction $w(x)$ des cas
Legendre et Chebyshev est un cas spécial de $w(x) = (1-x)^\alpha (1+x)^\beta$?
...
\]
- Cependant, selon les produits intérieurs classiques, les monômes, $1, x, x^2, \ldots$,
...
sont définie par $T_0 = 1$, $T_1 = x$, $T_n(x) = 2xT_n(x) - T_{n-1}(x)$, alors
\[
...
pour l'approximation des fonctions sur les intervalles bornés.
    ...
    un polynôme de degré $n$, telle que
    \[
      \lim_{n \to \infty} \max_{x\in [a,b]} |f^{(l)}(x) - p_n^{(l)}(x)| = 0.
    \]

- 
- Les produits interieurs sont difficiles
    - rarement possible de façon analytique
    - les intégrales numérique sont coûteux
