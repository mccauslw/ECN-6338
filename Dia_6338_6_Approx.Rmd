...
### Approximations locales, basées sur les dérivées à un seul point
...
au point $x_0$.
- L'expansion a $n+1$ coefficients.
- Peu importe l'ordre $n$, l'approximation est locale: il existe $h_n(x)$ telle que
\[
  f(x) - \hat{f}(x) = h_n(x)(x-x_0)^n\,\mbox{et}\,\lim_{x\to 0} h_n(x) = 0.
\]
...
- Il faut faire attention aux zéros de $q(x)$ où l'approximation n'est pas bien définie.
...
- Nous passons maintenant aux expansions basées sur les évaluations sur une grille :
...
- Deux propriétés possibles :
    - partition d'unité : $\sum_{i=1} \phi_i(x) = 1$ pour $x \in [x_i,x_n)$
...
## MCO avec colonnes orthogales et l'approximation
